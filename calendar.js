            ...
            for (let k = 0; k < 7; k++) {
                ...
                if (j == 1) {
                    if (firstdate.toString().split(" ")[0] == weekdays[k].shortDay) {
                        ...
                        content += " <td id='_" + (i + 1) + "_ask_" + (displaynum) + "' onclick='askevent( " + displaynum + "," + (i + 1) + " )'> <h3>" + displaynum + "</h3></td>";
                        j++;
                    }
                    else {
                        content += "<td></td>";
...
function askevent(num, id) {
    ...
    console.log(num);
    if (c != null) {
        document.getElementById("_" + id + "_ask_" + num).innerHTML = "<h2>" + num + "</h2> <h2 style='color : red'>" + c + "</h2>";
